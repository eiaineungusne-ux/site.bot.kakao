...
test('public status, safe text rendering and API failure', async () => {
  ...
  w.fetch=async url=>{
    ...
    return {ok:true,json:async()=>String(url).includes('status-config')?{apiBase:'https://status.example'}:{
      state:'temporary_ban',title:'카카오톡 임시제한',message:'복구 중',lastSeen:1000,checkedAt:Date.now(),notice:injection,hours:{},
      incidents:[{title:injection,resolvedAt:null,updates:[{at:1000,stage:'investigating',message:injection}]}],
    }};
  };
  try {
    w.eval(code); await flush();
    assert.equal(w.document.getElementById('status-title').textContent,'카카오톡 임시제한');
    assert.equal(w.document.querySelectorAll('#history-bars span').length,24);
...
test('administrator can load, publish a notice and clear credentials on logout', async () => {
  ...
  const w=dom.window; w.AbortSignal=AbortSignal; w.setInterval=()=>0; const calls=[];
  w.fetch=async(url,options)=>({ok:true,json:async()=>{
    ...
    return {notice:payload.message||'',manual:payload.action==='display'?{state:payload.state,message:payload.message}:null,incidents:[]};
  }});
  try {
    ...
    w.document.getElementById('display-state').value='temporary_ban';
    w.document.getElementById('display-form').dispatchEvent(new w.Event('submit',{cancelable:true})); await flush();
    assert.deepEqual(calls[3].payload,{action:'display',state:'temporary_ban',message:''});
